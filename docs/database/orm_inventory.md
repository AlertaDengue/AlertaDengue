...
| `historico_casos` | materialized view | legacy | archive | do-not-map | read-only | `dengueadmin` | none | no runtime evidence found | no PK; approx. rows `4796063`; size `358580224` bytes | PR `#1011` removed the legacy application dependency by replacing the combined dashboard query with disease-specific `Historico_alerta*` queries, and current repository search found no active read or write path left. Repository SQL history now implements the reviewed `archive_historico_casos` batch that moves the materialized view to `archive_historico_casos.historico_casos` while preserving indexes, ownership, and ACLs. The full `archive -> validate -> restore -> archive -> validate` cycle was validated locally on 2026-07-27 in a disposable PostgreSQL database, with latest archived `data_iniSE` = `2026-07-05`; staging and production remain unchanged. |
