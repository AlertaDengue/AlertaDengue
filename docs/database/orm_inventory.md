...
| `Notificacao` | table | active | retain | map-unmanaged | read-write-application | `administrador` | none | raw SQL and ingestion UPSERT | PK `id`; unique `(nu_notific, dt_notific, cid10_codigo, municipio_geocodigo)`; approx. rows `37097472`; size `13733748736` bytes | Queried by `api/internal/services.py` and `api/db.py`; written by `ingestion/tasks.py` merge logic. No current Django model exists for this table, so intended ORM work remains separate from current ownership. The legacy public literal-name staging table `public."""Municipio"".""Notificacao"""` was audited separately as a strict `A90` 2022 subset of this active table and now has a reviewed physical-removal batch under `containers/postgres/sql_history/notificacao_staging_removal/`. |
...
| `Tweet` | table | legacy | archive | do-not-map | unknown | `administrador` | none | no runtime evidence found | PK `id`; FK `CID10_codigo -> Dengue_global.CID10(codigo)`; approx. rows `3879263`; size `317546496` bytes | `Municipio.Tweet` stored daily aggregated dengue tweet counts, not raw tweet content. The frozen dataset covers `2012-08-01` through `2022-09-05` and is retained under `archive_tweets."Tweet"` for historical reproducibility. Repository SQL history now adds archive, validation, and restoration scripts that move the table and owned sequence to `archive_tweets` while preserving PK `Tweet_pk`, index `Tweets_idx_data`, FK `Tweet_CID10`, default, ownership, ACLs, and comment. There is no active `AlertaDengue` runtime dependency, current `pipe_infodengue()` does not read tweets, and deprecated `AlertTools::getTweet()` remains transition-safe by preferring `archive_tweets."Tweet"` and falling back to `"Municipio"."Tweet"` during migration. Physical deletion is not authorized. |
...
- The `archive_historico_casos` batch is implemented in repository SQL
  history and validated in a disposable database, but production remains
  unchanged until an operator executes the reviewed scripts there.
...
## Related `public` legacy tables

- `public."""Municipio"".""Notificacao"""` is a literal-name staging or import
  table, not the active `"Municipio"."Notificacao"` relation. The preserved
  audit artifact in
  `containers/postgres/sql_history/public_municipio_notificacao/` verified that
  it contains `30000` `A90` rows from 2022 and is a strict subset of the
  active notification table after municipality normalization. ORM status:
  `do-not-map`. Retention: `delete`. The reviewed physical-removal batch now
  lives under `containers/postgres/sql_history/notificacao_staging_removal/`,
  but no staging or production deletion has been executed by this repository
  change.
