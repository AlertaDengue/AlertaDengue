...
BEGIN
  ...
  SELECT coalesce(array_agg(relname ORDER BY relname),'{}') INTO t FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE nspname='public' AND relkind='r' AND relname IN ('dbf_dbf','dbf_dbfchunkedupload','dbf_sendtopartner');
  ...
  SELECT coalesce(array_agg(relname ORDER BY relname),'{}') INTO s FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE nspname='public' AND relkind='S' AND relname IN ('dbf_dbf_id_seq','dbf_dbfchunkedupload_id_seq','dbf_sendtopartner_id_seq');
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE nspname='public' AND relname='auth_user') THEN RAISE EXCEPTION 'protected auth_user object missing'; END IF;
...
SELECT 'public.'||relname AS table_name, count(*) AS exact_rows, pg_total_relation_size(oid) AS total_bytes
FROM public.dbf_dbf UNION ALL SELECT 'public.dbf_dbfchunkedupload',count(*),pg_total_relation_size('public.dbf_dbfchunkedupload'::regclass) FROM public.dbf_dbfchunkedupload UNION ALL SELECT 'public.dbf_sendtopartner',count(*),pg_total_relation_size('public.dbf_sendtopartner'::regclass) FROM public.dbf_sendtopartner;
